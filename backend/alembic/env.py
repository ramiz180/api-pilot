"""
Alembic async migration environment for api-pilot.

Key design decisions:
- Database URL is read from app.config.Settings (never hardcoded here).
- target_metadata points to Base.metadata so autogenerate works.
- All model modules must be imported below so SQLAlchemy registers their
  tables on Base.metadata before Alembic compares against the live schema.
- Supports both offline (SQL script) and online (live DB) migration modes.
- Uses async engine + run_sync pattern required by SQLAlchemy 2.x async.
"""
...
from app import models  # noqa: F401  (empty package for now — populated in Sprint 1)
...
# Override the DB URL from settings (ignores the blank value in alembic.ini)
...
# ---------------------------------------------------------------------------
# Online migrations — apply against a live database
# ---------------------------------------------------------------------------
